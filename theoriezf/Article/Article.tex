...
\begin{abstract}
We show how to implement a version of set theory in \dedukti ~in such a way that it respects normalization. To do so, we adapt in $\lambda \Pi$\textit{--calculus modulo theory} the guideline given in \textit{Deduction modulo} by G. Dowek and A. Miquel --using a theory of pointed graphs-- and we \textit{formally} write the proofs in \dedukti. To achieve this goal, we develop a language of formulas.
\end{abstract}
...
To implement set theory in \dedukti, the first idea is to translate the axioms of the theory into rewriting rules. However, as pointed out by M. Crabbé \cite{crabbé}, such implementation does not guarantee normalization. That is why G. Dowek and A. Miquel \cite{zermodulo} introduced a structure of pointed graphs to encode set theory. This set theory with pointed graphs -- called IZmod -- respects normalization.

\subsection{Sets as pointed graphs}

\label{informal}

In the IZmod theory \cite[see][Section 3.1]{zermodulo}, sets are reprented by pointed graphs. A pointed graph is a direct graph whose one of its node is identified as the root. 
...
We see in the examples that some different pointed graphs can represent the same set. To characterized this, G. Dowek and A. Miquel introduce bisimilarity between graphs, noted $\simeq$, and with rewriting rule 
...
In set theory, there only exists one sort: the sets. In the IZmod theory, there exists two sorts: nodes and pointed graphs. Therefore, there are in IZmod two constructions related to the inclusion of sets. We can take the example of $\emptyset \in \{\emptyset\}$. We can represents this inclusion by two different ways.

\begin{figure}[h]
\centering
\begin{tikzpicture}[>=stealth,yscale=1.8,xscale=1.4]  

\node(a) at (3,2.75)[circle,draw,text centered,fill=gray] {h};
\node(k) at (3.5,2.75)[text centered]{$\in$};
\node(b) at (4,3)[circle,draw,text centered,fill=gray] {i};
\node(c) at (4,2.5)[circle,draw,text centered] {j};
\node(d) at (6,3)[circle,draw,text centered] {k};
\node(e) at (6,2.5)[circle,draw,text centered] {$\ell$};
...
Into a pointed graphs, inclusion is represented by the constructor $\eta$. Indeed, there exists an edge from $k$ to $\ell$, which means that the pointed graph with root $i$ is included in the pointed graph with root $h$. 

Between two pointed graphs, inclusion is represented by a new constructor noted $\in$, with rewriting rule $a \in b \lra \ex~x~(x~\eta_b~root(b)~\conj~a \simeq (b/x))$. \\
...
Many definitions of the set theory intervines. The two classical set theories are Zermelo (Z) and Zermelo-Fraenkel (ZF). The set theory defined in \textit{Deduction modulo} by G. Dowek and A. Miquel \cite[see][Section 2]{zermodulo} --called IZst-- is stronger than Zermelo set theory but weaker than Zermelo-Fraenkel set thery. Indeed, the IZst theory contains Zermelo set theory, the Strong Extensionality axiom and the Transitive Closure axiom. \\
...
Since the publication of \cite{zermodulo}, \lpcm ~has been developped as an evolution of \textit{Deduction modulo}. More specifically, \lpcm ~allows to quantify on proposition. In that respect, the schemes of axioms have been replaced by axioms, as henceforth we do not need to write one axiom for each proposition. \\

In \dedukti, the symbol $mem$ and $rel$ \cite[see][Table 2]{zermodulo} are useless. Indeed, we can apply a node $x$ to class $P$ with $P~x$ and two nodes $x$ and $y$ to a relation $r$ with $r~x~y$.
...
All the other rewriting rules \cite[see][Table 3]{zermodulo}, except the ones involving $comp$, are easy to implement in \dedukti.
...
The pointed graph $omega$ represents the set of Von Neumann ordinals \cite[see][Table 2]{zermodulo53}. Indeed, the informal presentation of the representation of sets by pointed graphs given in \ref{informal} corresponds to Von Neumann ordinals. The rewriting rule of $x~\eta_{omega}~x'$ states that in $omega$ the edges represents the order relation $<$ and that there is an edge from every natural number to the root of $omega$.


\section{The language of formulas}

We may have notice that the lemmas in which the $comp$ symbol is used are only valid for a subset of formulas \cite[see][Table 5]{zermodulo}. The formulas need to have all of its quantifiers of sort $El~graph$ and can only use the language $\in$, $\simeq$ and the classic logical connectives.
...
There are 53 lemmas to proved \textit{formally} \cite[see][Tables a and 5]{zermodulo}.
